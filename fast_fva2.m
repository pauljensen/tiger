function [minflux,maxflux] = fast_fva2(tiger,varargin)
...
function [range] = fva_aux(model,vars,bounds,statbar)
    ...
    alts.obj = zeros(length(model.obj),count(~fixed));
    alts.obj(vars(~fixed),:) = eye(count(~fixed));
    
    set_solver_option('MaxTime',single_solve_maxtime);
    
    sols = cmpi.solve_multiple_mips(model,alts);
    range(vars(~fixed)) = sols.val;
